"""Intelligent provider detection for verdict.
...
def detect_custom_endpoints() -> list[DetectedProvider]:
    ...
    # Check verdict config
    config_path = Path.home() / ".config" / "verdict" / "verdict.yaml"
            ...
            for name, provider in (config.get("providers") or {}).items():
                detected.append(
                    DetectedProvider(
                        ...
                        description=f"From verdict config: {provider.get('base_url')}",
                    )
                )
        except Exception:
            pass
...
def format_detection_report(result: DetectionResult, verbose: bool = False) -> str:
    ...
    lines = ["\n╔══════════════════════════════════════════════════════════════╗"]
    lines.append("║         verdict Provider Detection Report                 ║")
    lines.append("╚══════════════════════════════════════════════════════════════╝\n")

    all_providers = result.all_providers()

    if not all_providers:
        lines.append("No providers detected. Run 'verdict setup' to configure one.\n")
        return "\n".join(lines)
    ...
    if not has_router and (has_local or has_cloud):
        ...
        lines.append("    This makes verdict actually useful for routing between tiers.")
        lines.append("")
        lines.append("    Quick start:")
        lines.append("      npm install -g 9router")
        lines.append("      9router serve  # Runs on http://localhost:20128/v1")
        lines.append("      verdict setup  # Will auto-detect omniroute")
    elif has_router:
        lines.append("  ✅ Centralized router detected — verdict can route between")
