...
def load_data(path):
    ...
    def calc_cost(model):
        if "opus" in model: return 0.015
        if "sonnet" in model: return 0.003
        if "flash" in model or "8b" in model: return 0.00035
        return 0.001
...
with c2:
    ...
    df_time = df.set_index("ts").resample("1H")[["estimated_cost", "primary_cost_baseline"]].sum().reset_index()
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=df_time["ts"], y=df_time["primary_cost_baseline"], fill='tozeroy', name='Cost without llm-gate (Opus)', fillcolor='rgba(255,0,0,0.1)', line=dict(color='red', dash='dot')))
    fig2.add_trace(go.Scatter(x=df_time["ts"], y=df_time["estimated_cost"], fill='tozeroy', name='Actual Cost with llm-gate', fillcolor='rgba(0,255,0,0.2)', line=dict(color='green')))
    fig2.update_layout(title="Cumulative Spend Drift", hovermode='x unified')
    st.plotly_chart(fig2, use_container_width=True)

st.divider()
st.subheader("Raw Decision Log")
st.dataframe(df[["ts", "task_preview", "input_tier", "model_chosen", "reason", "savings"]].sort_values("ts", ascending=False), use_container_width=True)
