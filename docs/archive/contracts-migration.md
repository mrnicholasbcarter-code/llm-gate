...
`verdict` now emits versioned shared contracts in `schemas/contracts.v1.json`.
The JSON is intentionally usable by Python and `verdict-node`; unknown fields are
