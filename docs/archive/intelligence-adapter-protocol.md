...
This protocol defines the interfaces and invariants for all communication between the public `verdict` gateway and the underlying Ruflo/RuVector managed intelligence engines.

---

## 1. Compliance & Security Boundaries

1. **Protocol Isolation**: `verdict` interacts with Ruflo/RuVector exclusively through versioned subprocess CLI interfaces. The gateway **MUST NOT** directly access SQLite databases (`memory.db`/`state.db`) or raw database tables of the underlying runtimes.
