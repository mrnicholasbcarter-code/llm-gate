...
`verdict` is designed to be highly interoperable and integrates seamlessly with popular developer agents, proxies, and editor extensions.

---

## 1. Editor Integrations (Cursor & VSCode)

To utilize `verdict` within Cursor or VSCode:
...
For enterprise workspaces managing multiple developer API tokens, you can route `verdict` as an upstream model provider inside LiteLLM:

```yaml
model_list:
  - model_name: verdict-sol
    litellm_params:
...
## 3. Node.js & TypeScript Middleware (`verdict-node`)

Integrate `verdict` directly into Express or NextJS backend routes using the `@nickhq/verdict-node` package:

```typescript
import { createRouterMiddleware } from "@nickhq/verdict-node";
...
# For Claude Code
# Configure custom system hooks pointing to path/to/verdict binary
```
