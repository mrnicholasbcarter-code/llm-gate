# Tooling & Framework Stack Digest — verdict portfolio
...
verdict = deterministic eligibility/availability authority (the thing that makes
...
## 2. verdict (mrnicholasbcarter-code/verdict) — availability/eligibility authority

- Repo at /home/nick/dev/verdict. main green: 313 pytest, ruff clean, mypy --strict clean.
...
- `npx ruflo init` was run in verdict → wrote `.claude/`, `.claude-flow/config.yaml`,
...
- Real docs are at `verdict/.agents/skills/ruflo/plugins/*/README.md` (30+ plugin READMEs)
  and `verdict/.agents/skills/ruflo/AGENTS.md`. The npm `@metaharness` dir is an EMPTY STUB
...
- For verdict: could embed the model catalog + our routing decisions for "similar past
...
- Installed into verdict `.venv` (pip). Version 2.3.7. Repo: github.com/tirth8205/code-review-graph.
...
- Invoke via `.venv/bin/code-review-graph <cmd>` from verdict dir.
...
- Portfolio repos under /home/nick/dev: backtest-harness, edge-mining-framework,
  verdict-node, trade-risk-engine, trading-cockpit-ui, hermes-plugins, mrnicholasbcarter-code.
...
- ruflo: /home/nick/dev/verdict/.agents/skills/ruflo/AGENTS.md + plugins/*/README.md
  (esp ruflo-metaharness, ruflo-ruvector, ruflo-intelligence, ruflo-core).
- ruvector: /home/nick/dev/verdict/.agents/skills/ruflo/plugins/ruflo-ruvector/README.md.
...
- verdict: repo source (availability_cache.py, api.py, router.py, intelligence.py, omniroute.py).
