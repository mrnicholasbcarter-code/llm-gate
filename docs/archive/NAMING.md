# Rebrand Proposals — `verdict` Ecosystem

Status: proposal for review. No repos are renamed until a name is chosen.

## Why rename

`verdict` is generic and undersells the product. The stack is a
...
| --- | --- | --- |
| `verdict-core` | Routing/eligibility control plane (flagship) | `verdict-core` |
| `verdict-ecosystem` | Umbrella / meta repo | `verdict` |
| `verdict-strategy` | Edge Mining Framework | `verdict-edge` |
| `verdict-risk` | Trade Risk Engine | `verdict-risk` |
| `verdict-backtest` | Backtest Harness | `verdict-backtest` |
| `verdict-cockpit` | React/TS dashboard | `verdict-cockpit` |
| `verdict-node` | Node integration | `verdict-node` |
...
4. Keep `verdict` as a deprecated alias for one release for CLI users.
