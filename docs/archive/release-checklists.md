...
This document details the mandatory QA criteria and release gates that the `verdict` framework and all adjacent repositories must fulfill before any deployment or semantic release.
...
* **Unit Test Coverage**: Run `pytest` or Jest tests. Minimum test coverage thresholds:
  * `verdict`: 100% core routing modules.
  * `trade-risk-engine`: 100% precision drift and time gate modules.
  * `verdict-node`: complete SSE proxy and AbortController listener.
