...
`verdict` serves as a self-optimizing, token-efficient intelligence router. It sits as a thin proxy gateway between agent runtimes (Codex, Hermes, Claude Code) and model catalog providers (O‍mniRoute, OpenRouter), ensuring requests are routed with mathematical cost/performance efficiency.
...
```
   [ Client Request ]
           │
           ▼
[ verdict Proxy Gateway ] ◄─────► [ Deterministic Safety Gates ]
...
Before any routing takes place, `verdict` applies hard deterministic gates locally:
...
This fetches model rankings dynamically optimized via reinforcement learning (SONA). If the adapter is unavailable, `verdict` falls back gracefully to a configured local safety floor model in development mode, or fails closed in production mode.

---

## 3. Availability Adapter Boundary

The `verdict` catalog handles local normalizations for provider endpoints. Candidates are parsed using:
...
If a model fails a TCP connection lookup or returns transport errors, `verdict` quarantines the model and avoids dispatching tasks to it until the next lifecycle check.
