# ADR: Versioned, Privacy-Safe Execution Evidence

**Status:** Accepted for issue #53 implementation
**Date:** 2026-07-22
**Related:** `ADR-ORCHESTRATOR-ROUTING.md`; Ruflo ADR-103, ADR-131, ADR-144, ADR-150, ADR-171, ADR-176
...
claims. Caller-provided request IDs are also not safe as storage keys because
they can be reused concurrently.
...
Verdict will expose execution evidence as a versioned, tagged envelope around
the existing decision and outcome contracts. The envelope is selected through
an authenticated, scope-bound explain API and uses a server-generated opaque
